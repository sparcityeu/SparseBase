...
TEST(ReorderHeatmap, Instance) {
  ...
  heatmap = heatmapper.Get(&global_coo, cuda_arr, cuda_arr,
                           {global_csr.get_context()}, true);
...
TEST(ReorderHeatmap, InstanceTwoReorders) {
  ...
  auto heatmap = heatmapper.Get(&global_csr, &r_reorder_perm, &c_reorder_perm,
                                {global_csr.get_context()}, true);
  auto heatmap_arr = heatmap->As<format::Array>()->get_vals();
  for (int i = 0; i < 9; i++)
  ...
  heatmap = heatmapper.Get(&global_coo, cuda_arr_r, cuda_arr_c,
                           {global_csr.get_context()}, true);
  heatmap_arr = heatmap->As<format::Array>()->get_vals();
  for (int i = 0; i < 9; i++)
    EXPECT_EQ(heatmap_arr[i], heatmap_rc_order_true[i]);
