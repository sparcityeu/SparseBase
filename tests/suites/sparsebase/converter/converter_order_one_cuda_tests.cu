...
TEST_F(CUDAFormatsFixture, CUDAArray) {
  ...
  EXPECT_EQ(output_format->get_id(), (CUDAArray<int>::get_id_static()));
  ...
  EXPECT_EQ(output_format_csr->get_id(), (Array<int>::get_id_static()));
  compare_arrays_cuda_cpu(
      output_format->AsAbsolute<CUDAArray<int>>()->get_vals(),
...
TEST_F(CUDAFormatsFixture, CUDAArrayCached) {
  ...
  // Can't convert for bad context
  EXPECT_THROW(
      (c1.ConvertCached(array, CUDAArray<int>::get_id_static(), &cpu, false)),
      utils::ConversionException);

  output_formats =
      c1.ConvertCached(array, CUDAArray<int>::get_id_static(), &gpu0, false);
  ...
  output_formats = c1.ConvertCached(array, CUDAArray<int>::get_id_static(),
                                    {&gpu0, &cpu}, false);
  EXPECT_EQ(output_formats.size(), 1);
  compare_arrays_cuda_cpu(
      output_formats[0]->AsAbsolute<CUDAArray<int>>()->get_vals(),
...
TEST_F(CUDAFormatsFixture, CUDAArrayMultiContext) {
  ...
  output_format =
      c1.Convert(array, CUDAArray<int>::get_id_static(), {&gpu0, &cpu}, false);
  compare_arrays_cuda_cpu(
      output_format->AsAbsolute<CUDAArray<int>>()->get_vals(),
      array->get_vals(), array->get_num_nnz());
  delete output_format;
