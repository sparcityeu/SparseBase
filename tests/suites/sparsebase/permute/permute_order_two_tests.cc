...
TEST(PermuteTest, RowWise) {
  PermuteOrderTwo<int, int, int> transformer(r_reorder_vector, nullptr);
...
TEST(InversePermuteTest, RowColWise) {
  ...
  PermuteOrderTwo<int, int, int> transformer(inv_r_order, inv_c_order);
  auto transformed_format =
...
TEST(PermuteTest, RowColWise) {
  PermuteOrderTwo<int, int, int> transformer(r_reorder_vector,
                                             c_reorder_vector);
  auto transformed_format =
      transformer.GetPermutation(&global_coo, {&cpu_context}, true)
          ->As<format::CSR>();
