...
protected:
  ...
  void SetUp() override{
    coo = new COO<vertex_type, edge_type, value_type>(6, 6, 6, rows, cols, vals, kNotOwned);
  }
...
TEST(feature, ExtractorAddSub){
  ...
  extractor.Add(Feature(Degrees_DegreeDistribution<vertex_type, edge_type, value_type, feature_type>{}));
  list = extractor.GetList();
  EXPECT_EQ(list.size(), 2);

  extractor.Subtract(Feature(DegreeDistribution<vertex_type, edge_type, value_type, feature_type>{}));
...
TEST(feature, FeatureExtractorRegisterMap){
  std::vector<type_index> expected = {
    DegreeDistribution<vertex_type, edge_type, value_type, feature_type>::get_feature_id_static(),
    Degrees<vertex_type, edge_type, feature_type>::get_feature_id_static(),
    Degrees_DegreeDistribution<vertex_type, edge_type, value_type, feature_type>::get_feature_id_static()
  };
...
TEST_F(COOMock, ExtractorExtract){
  ...
  Extractor engine = FeatureExtractor<vertex_type, edge_type, value_type, feature_type>();
  engine.Add(Feature(Degrees<vertex_type, edge_type, value_type>{}));

  auto raw = engine.Extract(coo, {&cpu_context});
  EXPECT_EQ(raw.size(), 1);
  auto degrees = any_cast<vertex_type*>(raw[Degrees<vertex_type, edge_type, value_type>::get_feature_id_static()]);

  engine.Add(Feature(DegreeDistribution<vertex_type, edge_type, value_type, feature_type>{}));
  auto raw2 = engine.Extract(coo, {&cpu_context});
  EXPECT_EQ(raw2.size(), 2);
  auto degree_dist = any_cast<feature_type *>(raw2[DegreeDistribution<vertex_type, edge_type, value_type, feature_type>::get_feature_id_static()]);
...
//      new DegreeDistribution<vertex_type, edge_type, value_type, feature_type>();
...
//      new Degrees_DegreeDistribution<vertex_type, edge_type, value_type, feature_type>();
...
//  in.insert(make_pair(DegreeDistribution<vertex_type, edge_type, value_type, feature_type>::get_feature_id_static(), new DegreeDistribution<vertex_type, edge_type, value_type, feature_type>()));
//  in.insert(make_pair(Degrees<vertex_type, edge_type, feature_type>::get_feature_id_static(), new Degrees<vertex_type, edge_type, feature_type>()));
//
//  vector<ExtractableType*> rs = cmm.GetClasses(in);
//  EXPECT_EQ(rs.size(), 1);
//  EXPECT_EQ(rs[0]->get_feature_id(), degrees_degreedistribution->get_feature_id());
//
//  in.insert(make_pair(Degrees_DegreeDistribution<vertex_type, edge_type, value_type, feature_type>::get_feature_id_static(), new Degrees_DegreeDistribution<vertex_type, edge_type, value_type, feature_type>()));
//  rs = cmm.GetClasses(in);
//  EXPECT_EQ(rs.size(), 2);
//  EXPECT_EQ(rs[0]->get_feature_id(), degrees_degreedistribution->get_feature_id());
//  EXPECT_EQ(rs[1]->get_feature_id(), degrees_degreedistribution->get_feature_id());
//}
...
TEST(feature, Feature){
  ...
  DegreeDistribution<vertex_type, edge_type, value_type, feature_type> degree_dist;
