...
TEST_F(Degrees_DegreeDistributionTest, FeaturePreprocessTypeTests) {
  ...
  EXPECT_EQ(feature.get_params(feature::Degrees<int, int, int>::get_id_static())
                .get(),
            p1.get());
  EXPECT_NE(feature.get_params(feature::Degrees<int, int, int>::get_id_static())
                .get(),
            p2.get());
...
TEST_F(Degrees_DegreeDistributionTest, Degree_DegreeDistributionTests) {
  ...
  ASSERT_NO_THROW(std::any_cast<float *>(
      degrees_and_distribution_map[feature::DegreeDistribution<
