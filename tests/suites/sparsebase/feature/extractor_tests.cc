...
TEST(feature, ExtractorAddSub) {
  ...
  extractor.Add(
      Feature(feature::Degrees_DegreeDistribution<vertex_type, edge_type,
                                                  value_type, feature_type>{}));
  list = extractor.GetList();
  EXPECT_EQ(list.size(), 2);

  extractor.Subtract(
      Feature(feature::DegreeDistribution<vertex_type, edge_type, value_type,
                                          feature_type>{}));
  extractor.Subtract(
      Feature(feature::Degrees<vertex_type, edge_type, value_type>{}));
  list = extractor.GetList();
...
TEST_F(COOMock, ExtractorExtract) {
  ...
  engine.Add(Feature(feature::DegreeDistribution<vertex_type, edge_type,
                                                 value_type, feature_type>{}));
