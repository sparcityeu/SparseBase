...
class MedianDegreeTest : public ::testing::Test {
 protected:
  MedianDegree<int, int, int, float> feature;

  struct Params1 : sparsebase::utils::Parameters {};
  struct Params2 : sparsebase::utils::Parameters {};
};

TEST_F(MedianDegreeTest, AllTests) {
  // test get_sub_ids
  ...
  // Check GetMedianDegreeCSR implementation function
  Params1 p1;
  auto median_degree =
      MedianDegree<int, int, int, float>::GetMedianDegreeCSR(
          {&global_csr}, &p1);

  std::vector<int> test_degrees;
  test_degrees.reserve(n);
  for (int i = 0; i < n; i++) {
    int current_degree = rows[i + 1] - rows[i];
  ...
  median_degree =
      feature.GetMedianDegree(&global_csr, {&cpu_context}, true);
  EXPECT_NEAR(*median_degree, median_in_degrees, 0.000001);
  delete median_degree;

  median_degree =
      feature.GetMedianDegree(&global_csr, {&cpu_context}, false);
  EXPECT_NEAR(*median_degree, median_in_degrees, 0.000001);
  delete median_degree;

  // Check GetMedianDegree with conversion
  median_degree =
      feature.GetMedianDegree(&global_coo, {&cpu_context}, true);
  EXPECT_NEAR(*median_degree, median_in_degrees, 0.000001);
  delete median_degree;
  EXPECT_THROW(feature.GetMedianDegree(&global_coo, {&cpu_context}, false),
               utils::DirectExecutionNotAvailableException<
                   std::vector<std::type_index>>);
  // Check GetMedianDegree with conversion and cached
  auto median_degree_format =
      feature.GetMedianDegreeCached(&global_coo, {&cpu_context}, true);
