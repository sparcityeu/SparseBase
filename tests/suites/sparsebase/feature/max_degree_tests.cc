...
class MaxDegreeTest : public ::testing::Test {
 protected:
  feature::MaxDegree<int, int, int> feature;

  struct Params1 : sparsebase::utils::Parameters {};
  struct Params2 : sparsebase::utils::Parameters {};
};

TEST_F(MaxDegreeTest, AllTests) {
  // test get_sub_ids
  ...
  Params1 p1;
  auto max_degree =
      feature::MaxDegree<int, int, int>::GetMaxDegreeCSR({&global_csr}, &p1);
  ...
  delete max_degree;
  // Check GetMaxDegree
  max_degree = feature.GetMaxDegree(&global_csr, {&cpu_context}, true);
  EXPECT_EQ(*max_degree, max_in_degrees);
  delete max_degree;

  max_degree = feature.GetMaxDegree(&global_csr, {&cpu_context}, false);
  EXPECT_EQ(*max_degree, max_in_degrees);
  delete max_degree;

  // Check GetMaxDegree with conversion
  max_degree = feature.GetMaxDegree(&global_coo, {&cpu_context}, true);
  EXPECT_EQ(*max_degree, max_in_degrees);

  EXPECT_THROW(feature.GetMaxDegree(&global_coo, {&cpu_context}, false),
