...
class BandwidthTest : public ::testing::Test {
 protected:
  feature::Bandwidth<int, int, void> feature;
  
  struct Params1 : sparsebase::utils::Parameters {};
};

TEST_F(BandwidthTest, AllTests) {
  ...
  // Check GetBandwidthCSR implementation function
  int* bandwidth =
      feature::Bandwidth<int, int, void>::GetBandwidthCSR({csr}, &p1);

  EXPECT_EQ(*bandwidth, ans);
  delete bandwidth;

  // Check GetBandwidth
  bandwidth = feature.GetBandwidth(csr, {&cpu_context}, true);
  EXPECT_EQ(*bandwidth, ans);
  delete bandwidth;

  bandwidth = feature.GetBandwidth(csr, {&cpu_context}, false);
  EXPECT_EQ(*bandwidth, ans);
  delete bandwidth;
