...
EXPECT_NO_THROW(EXECUTE_AND_DELETE(
    ReorderBase::Reorder<RCMReorder>(
        {}, &global_csr, {&cpu_context}, true)));
auto order = ReorderBase::Reorder<RCMReorder>(
    {}, &global_csr, {&cpu_context}, true);
...
TEST(ReorderBase, MetisReorder) {
  if(typeid(metis::idx_t) == typeid(int)) {
    EXPECT_NO_THROW(EXECUTE_AND_DELETE(
        ReorderBase::Reorder<MetisReorder>(
            {}, &global_csr, {&cpu_context}, true)));
    auto order = ReorderBase::Reorder<MetisReorder>(
        {}, &global_csr, {&cpu_context}, true);
    check_reorder(order, n);
  } else {
...
TEST(ReorderBase, RabbitReorder) {
  EXPECT_NO_THROW(EXECUTE_AND_DELETE(
      ReorderBase::Reorder<RabbitReorder>(
          {}, &global_csr, {&cpu_context}, true)));
  auto order = ReorderBase::Reorder<RabbitReorder>(
      {}, &global_csr, {&cpu_context}, true);
  check_reorder(order, n);
}
#endif
...
TEST(ReorderBase, AMDReorder) {
  EXPECT_NO_THROW(EXECUTE_AND_DELETE(
      ReorderBase::Reorder<AMDReorder>(
          {}, &global_csr, {&cpu_context}, true)));
  auto order = ReorderBase::Reorder<AMDReorder>(
      {}, &global_csr, {&cpu_context}, true);
  check_reorder(order, n);
}
#endif
...
EXPECT_NO_THROW(EXECUTE_AND_DELETE(
    ReorderBase::Reorder<DegreeReorder>(
        {true}, &global_csr, {&cpu_context}, true)));
auto order = ReorderBase::Reorder<DegreeReorder>(
...
EXPECT_NO_THROW(EXECUTE_AND_DELETE(
    ReorderBase::Reorder<GrayReorder>(
...
auto out_no_convert =
    ReorderBase::ReorderCached<RCMReorder>(
        {}, &global_csr, {&cpu_context});
EXPECT_EQ(out_no_convert.first.size(), 0);
auto order = ReorderBase::ReorderCached<RCMReorder>(
...
EXPECT_NO_THROW(ReorderBase::Permute1D(
...
inv_arr = ReorderBase::Permute1D<format::Array>(
    inverse_perm_array, &orig_arr, {&cpu_context}, true);

for (int i = 0; i < n; i++) {
EXPECT_EQ(inv_arr->get_vals()[i], reordered_array[i]);
}
EXPECT_THROW(ReorderBase::Permute2D<TestFormat>(
...
EXPECT_THROW(
    ReorderBase::Permute1D<TestFormatOrderOne>(
...
EXPECT_THROW(ReorderBase::Permute1D(
...
context::CUDAContext g0{0};
  ...
  auto output_convert_input_output =
      ReorderBase::Permute1DCached<format::Array>(
          inverse_perm_array, cuda_arr, {&cpu_context});
...
EXPECT_EQ((ReorderBase::Permute2D<format::CSR>(
    r_reorder_vector, &global_csr, {&cpu_context}, true))
->get_id(),
(format::CSR<int, int, int>::get_id_static()));
transformed_format =
ReorderBase::Permute2D<format::CSR>(
    r_reorder_vector, &global_csr, {&cpu_context}, true);
confirm_renumbered_csr(
    global_csr.get_row_ptr(), transformed_format->get_row_ptr(),
    global_csr.get_col(), transformed_format->get_col(), r_reorder_vector, n);
...
EXPECT_THROW(
    ReorderBase::Permute2DCached<format::COO>(
...
auto perm = ReorderBase::InversePermutation(
...
// check output of permutation
auto transformed_format =
    ReorderBase::Permute2DRowColumnWise(
        r_reorder_vector, c_reorder_vector, &global_csr, {&cpu_context}, true)
        ->Convert<format::CSR>();
for (int i = 0; i < n + 1; i++) {
...
EXPECT_NO_THROW(ReorderBase::Permute2DRowColumnWise(
    r_reorder_vector, c_reorder_vector, &global_csr, {&cpu_context}, true));
EXPECT_EQ((ReorderBase::Permute2DRowColumnWise(
...
// no conversion of output
auto output_no_convert_input =
    ReorderBase::Permute2DRowColumnWiseCached(
...
// no conversion of output
EXPECT_NO_THROW(ReorderBase::Permute2DRowWise(
    r_reorder_vector, &global_csr, {&cpu_context}, true));
// check output of permutation
auto transformed_format =
    ReorderBase::Permute2DRowWise(
        r_reorder_vector, &global_csr, {&cpu_context}, true)
        ->Convert<format::CSR>();
for (int i = 0; i < n + 1; i++) {
...
EXPECT_NO_THROW(ReorderBase::Permute2DRowWise(
    r_reorder_vector, &global_csr, {&cpu_context}, true));
EXPECT_EQ((ReorderBase::Permute2DRowWise(
    r_reorder_vector, &global_csr, {&cpu_context}, true))
->get_id(),
(format::CSR<int, int, int>::get_id_static()));
...
// converting output to illegal format (boolean is off)
EXPECT_THROW(
    ReorderBase::Permute2DRowWise<TestFormat>(
    r_reorder_vector, &global_csr, {&cpu_context}, true, false),
    utils::TypeException);
// converting output to illegal format (No conversion available)
EXPECT_THROW(
...
// no conversion of output
auto output_no_convert_input =
    ReorderBase::Permute2DRowWiseCached(
        r_reorder_vector, &global_csr, {&cpu_context});
EXPECT_EQ(output_no_convert_input.first.size(), 0);
// converting input to possible format
auto output_convert_input =
