...
TEST(GraphFeatureBase, Degrees) {
  EXPECT_NO_THROW(
      GraphFeatureBase::GetDegrees(&global_csr, {&cpu_context}, true));
  auto degrees_array =
      GraphFeatureBase::GetDegrees(&global_csr, {&cpu_context}, true);
...
TEST(GraphFeatureBase, DegreesCached) {
  EXPECT_NO_THROW(
      GraphFeatureBase::GetDegreesCached(&global_csr, {&cpu_context}));
  auto output = GraphFeatureBase::GetDegreesCached(&global_csr, {&cpu_context});
  ...
  auto output_conv =
      GraphFeatureBase::GetDegreesCached(&global_coo, {&cpu_context});
...
TEST(GraphFeatureBase, DegreeDistribution) {
  EXPECT_NO_THROW(GraphFeatureBase::GetDegreeDistribution<float>(
      &global_csr, {&cpu_context}, true));
  auto degreeDistribution_array =
      GraphFeatureBase::GetDegreeDistribution<float>(&global_csr,
                                                     {&cpu_context}, true);
...
TEST(GraphFeatureBase, DegreeDistributionCached) {
  EXPECT_NO_THROW(GraphFeatureBase::GetDegreeDistributionCached<float>(
      &global_csr, {&cpu_context}));
  auto output = GraphFeatureBase::GetDegreeDistributionCached<float>(
      &global_csr, {&cpu_context});
  ...
  EXPECT_EQ(output.first.size(), 0);
  auto output_conv = GraphFeatureBase::GetDegreeDistributionCached<float>(
