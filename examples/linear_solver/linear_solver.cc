...
// std::tuple<val, val, ull> compare(T* j1, T* j2, C count, T threshold, bool
// verbose = false, bool print_samples = false);
...
int main() {
  ...
  ull *perm = bases::ReorderBase::Reorder<reorder::RCMReorder>(
      {}, A, {&cpu_context}, true);
  ...
  ull *inv_perm =
      bases::ReorderBase::InversePermutation(perm, A->get_dimensions()[0]);
  format::Array<val> *x = bases::ReorderBase::Permute1D<Array>(
      inv_perm, inv_x, {&cpu_context}, true);
...
//
// template<typename T, typename C>
// std::tuple<double, double, ull> compare(T* j1, T* j2, C count, T threshold,
// bool verbose, bool print_samples){
//  std::vector<C> indexes;
//  std::tuple<double, double, ull> comparison_tuple= compare_vectors(j1, j2,
//  count,  threshold, indexes); if (indexes.size() > 0){
//    std::cout << "Errors:\n";
//    for (int i =0; i<indexes.size() && i < 10; i++){
//      std::cout << indexes[i] << " " << j1[indexes[i]] << " " <<
//      j2[indexes[i]] << std::endl;
//    }
//  }
//  if (verbose){
//    std::cout << "Mean = " << std::get<0>(comparison_tuple) << " STD = " <<
//    std::get<1>(comparison_tuple)  << "Tot. diff. = " <<
//    std::get<2>(comparison_tuple) << std::endl; if (print_samples){
//      std::cout << "Samples:\n";
...
std::cout << "  Rows: " << A_csc->get_dimensions()[0] << ". Columns: " <<
A_csc->get_dimensions()[1] << std::endl; std::cout << "  Number of non-zeros: "
<< A_csc->get_num_nnz() << std::endl; std::cout << "b properties:" << std::endl;
...
arma::SpMat<double> a_mat(row, col_ptr, vals, A->get_dimensions()[0],
A->get_dimensions()[1]); arma::dvec b_vals(b_reordered->get_vals(),
A->get_dimensions()[0]);
