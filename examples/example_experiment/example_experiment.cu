...
std::any spmv(unordered_map<string, Format *> &data, std::any &fparams,
  ...
  auto spm = data["processed_format"]
                 ->AsAbsolute<CSR<row_type, nnz_type, value_type>>();
...
// omp parallel spmv
std::any spmv_par(unordered_map<string, Format *> &data, std::any &fparams,
                  std::any &pparams, std::any &kparams) {
  auto v_arr = any_cast<Array<float>>(fparams);
  auto vals_arr = any_cast<Array<float>>(kparams);
  auto vals = vals_arr.get_vals();
...
__global__ void spmv_1t1r(nnz_type *row_ptr, row_type *cols, float *vals,
                          float *v, float *res, row_type n) {
...
__global__ void spmv_1w1r(nnz_type *row_ptr, row_type *cols, float *vals,
                          float *v, float *res, row_type n) {
  ...
  for (; rid < n; rid += (NUM_THREADS) / WARP_SIZE) {
    s = 0;
    for (unsigned int j = row_ptr[rid] + ridx; j < row_ptr[rid + 1];
         j += WARP_SIZE) {
      s += vals[j] * v[cols[j]];
    }
...
std::any spmv_gpu(unordered_map<string, Format *> &data, std::any &fparams,
                  std::any &pparams, std::any &kparams) {
  ...
  spmv_1t1r<<<NUM_BLOCKS, THREADS_PER_BLOCK>>>(rows, cols, c_vals->get_vals(),
                                               c_v->get_vals(), c_r->get_vals(),
                                               num_rows);
...
// many-core accelerated spmv
std::any spmv_gpu2(unordered_map<string, Format *> &data, std::any &fparams,
                   std::any &pparams, std::any &kparams) {
  CUDAContext gpu_context{0};
  auto s_v = any_cast<Array<float>>(fparams);
  auto c_v = s_v.Convert<CUDAArray>(&gpu_context);
  ...
  spmv_1w1r<<<NUM_BLOCKS, THREADS_PER_BLOCK>>>(rows, cols, c_vals->get_vals(),
                                               c_v->get_vals(), c_r->get_vals(),
                                               num_rows);
...
int main(int argc, char **argv) {
  ...
  exp.AddDataLoader(
      experiment::LoadCSR<MTXReader, row_type, nnz_type, value_type>,
      {make_pair(files, ash_v)});
  files = {argv[2]};
  exp.AddDataLoader(
      experiment::LoadCSR<EdgeListReader, row_type, nnz_type, value_type>,
      {make_pair(files, dblp_v)});

  // reorder matrices
  exp.AddPreprocess(
      "original", experiment::Pass,
  ...
  exp.AddPreprocess("RCM",
                    experiment::Reorder<reorder::RCMReorder, CSR, CPUContext,
                                        row_type, nnz_type, value_type>,
                    params);
