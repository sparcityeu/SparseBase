...
//! A single conversion step composed of a conversion function and a context to
//! use for the function
typedef std::tuple<ConversionFunction, context::Context *, utils::CostType>
    ConversionStep;

//! A chain of conversion functions-context pairs to be applied sequentially to
//! a single formats
...
//! A map containing conversion functions between format classes. Each function
//! is associated with a conditional function.
...
class Converter {
 ...
 public:
  ...
  /*!
   * Registers a conversion function from one type to another. The conversion
   * will be conditional on `edge_condition` which is a function that returns a
   * boolean based on the source and destination contexts of the conversion.
   * @param from_type std::type_index of the source format
   * @param to_type std::type_index of the destination format
   * @param conv_func a conversion function taking an input Format* and a
   * destination context.
   * @param edge_condition an std::function containing a boolean function with
   * two Context* parameters for the source and destination contexts. Returns
   * true if the conversion should happen based on context inputs.
   ...
   * context namespace for more information)
   * @param is_move_conversion if true the underlying arrays will be moved
   * instead of being copied
   * @return a pointer to the converted Format instance
   */
  ...
  /*!
   * Takes a single (format, context) input and returns the chain of conversions
   * needed to convert the input to the type `to_type`. The chain is optional,
   * meaning it could be empty if no conversions exist.
   * @param from_type type of source format
   * @param from_context context of source format
   * @param to_type format type to convert to
   * @param to_contexts contexts available for conversion
   * @param is_move_conversion whether to find move conversions or normal
   * conversions
   * @return an optional chain of conversion. If nothing is returned, then no
   * conversions are available. Otherwise, will return a tuple with its second
   * element being the total conversion cost, and its first element a vector of
   * (ConversionFunction, Context) pairs, where the input must be passed through
   * this chain starting from its front to its back to become `to_type`.
   */
  ...
  /*! Removes conversion functions that convert from `from_type` to `to_type`
   * from the conversion map
   * @param from_type type_index of the original format type
   * @param to_type type_index of the destination format type
   * @param move_conversion whether the conversions to be removed are move
   * conversions or not
   */
  ...
  /*! Removes all conversion functions from the current converter in all
   * directions.
   * @param move_conversion whether the conversions to be removed are move
   * conversions or not
   */
  ...
  /*!
   * Takes a conversion schema to convert all the formats in `packed_sfs`,
   * carries out the conversions, and returns a chain of converted formats for
   * each format. Chain contains original format, all intermediate formats, and
   * last format in the chain.
   * @param cs ConversionSchema containing the an optional chain of conversion
   * functions for every format in `packed_sfs`
   * @param packed_sfs formats to convert
   * @param clear_intermediate delete intermediate formats between the starting
   * and destination one.
   * @return a vector of size `packed_sfs.size()`. Each element is a vector of
   * size >= 1 containing the starting format in the conversion chain, every
   * intermediate format, and the final format. The first format is the original
   * and the last is the output format in each chain.
   */
