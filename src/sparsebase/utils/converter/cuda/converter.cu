...
CUDAArrayArrayConditionalFunction(Format *source, context::Context*context) {
  context::cuda::CUDAContext* gpu_context = static_cast<context::cuda::CUDAContext*>(source->get_context());
...
ArrayCUDAArrayConditionalFunction(Format *source, context::Context*context) {
  context::cuda::CUDAContext* gpu_context = static_cast<context::cuda::CUDAContext*>(context);
  ...
  return new format::cuda::CUDAArray<ValueType>(array->get_num_nnz(), vals, *gpu_context);
}
...
CsrCUDACsrConditionalFunction(Format *source, context::Context*context) {
  ...
  return new format::cuda::CUDACSR<IDType, NNZType, ValueType>(csr->get_dimensions()[0], csr->get_dimensions()[0], csr->get_num_nnz(), row_ptr, col, vals, *gpu_context);
}
template <typename IDType, typename NNZType, typename ValueType>
Format *
CUDACsrCUDACsrConditionalFunction(Format *source, context::Context*context) {
  context::cuda::CUDAContext* dest_gpu_context = static_cast<context::cuda::CUDAContext*>(context);
  auto cuda_csr = source->As<format::cuda::CUDACSR<IDType, NNZType, ValueType>>();
  context::cuda::CUDAContext* source_gpu_context = static_cast<context::cuda::CUDAContext*>(cuda_csr->get_context());
  ...
  return new format::cuda::CUDACSR<IDType, NNZType, ValueType>(cuda_csr->get_dimensions()[0], cuda_csr->get_dimensions()[0], cuda_csr->get_num_nnz(), row_ptr, col, vals, *dest_gpu_context);
}
template <typename IDType, typename NNZType, typename ValueType>
Format *
