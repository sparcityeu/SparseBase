...
public:
   ...
   * @tparam IDType type to represent the number of rows and columns in the object.
   * @tparam NNZType type to represent the number of non-zeros in the object.
   * @tparam ValueType type to represent the data inside the matrix (vertex weights in the case of a graph).
   * @param filename the path to the matrix market file.
   * @param convert_index_to_zero whether to convert indices in the matrix market to zero-indexing or to read them as is and add an empty zeroth row and column.
  ...
  /*!
   * Read the matrix market file in the path `filename` into a `Array<ValueType>` object. If the matrix inside the file
   * has more than a single row or more than a single column, an exception will be thrown.
   * @tparam IDType type to represent the number of rows and columns in the object.
   * @tparam NNZType type to represent the number of non-zeros in the object.
   * @tparam ValueType type to represent the data inside the matrix (vertex weights in the case of a graph).
   * @param filename the path to the matrix market file.
   ...
   * @param read_undirected if true, reading the edge (u,v) will also add the edge (v,u) to the object.
