 ...
 * containing an equal number of rows and columns, and calculates the density of
 * non-zeros in each cell in the grid relative to the total number of non-zeros
 * in the matrix, given that the matrix was reordered according to a permutation
 * matrix. Returns the densities as a dense array (FormatOrderOne) of size
 * num_parts * num_parts where the density at cell [i][j] in the 2D grid is
 * located at index [i*num_parts+j] in the grid. The density values sum up to 1.
