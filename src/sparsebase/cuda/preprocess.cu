...
format::Format * preprocess::JaccardWeights<IDType, NNZType, ValueType, FeatureType>::GetJaccardWeightCUDACSR(std::vector<Format *> formats, PreprocessParams * params){
    auto cuda_csr = formats[0]->As<CUDACSR<IDType, NNZType, ValueType>>();
    context::CUDAContext* gpu_context = static_cast<context::CUDAContext*>(cuda_csr->get_context());
    ...
    grid.x = a;
    grid.y = max((int)min((int)cuda_csr->get_dimensions()[0], (int)MAX_GRID_DIM), (int)1);
    grid.z = (int)min((int)max((int)cuda_csr->get_dimensions()[0] / grid.y, (int)1), (int)MAX_GRID_DIM);

    jac_binning_gpu_u_per_grid_bst_kernel<IDType, NNZType, FeatureType><<<grid, block, 0>>>(cuda_csr->get_row_ptr(), cuda_csr->get_col(), (NNZType)cuda_csr->get_dimensions()[0], jaccard_weights, 0);
    cudaDeviceSynchronize();
    auto array = new CUDAArray<FeatureType>(cuda_csr->get_num_nnz(), jaccard_weights, *gpu_context);
    return array;
}
...
__device__ inline NNZType bst(const NNZType* __restrict__ xadj, const IDType* __restrict__ adj, IDType neighbor, IDType target){
...
__global__ void jac_binning_gpu_u_per_grid_bst_kernel(const NNZType* xadj, const IDType* adj, NNZType n, FeatureType*  emetrics, IDType SM_FAC) {
  ...
  for(NNZType ptr = blockIdx.y+blockIdx.z*gridDim.y; ptr < n; ptr += gridDim.y*gridDim.z) {
    IDType u = ptr;
    NNZType degu = xadj[u+1] - xadj[u];

    for (NNZType neigh_ptr = threadIdx.y + blockDim.y * threadIdx.z + blockIdx.x*blockDim.y*blockDim.z; neigh_ptr< degu; neigh_ptr+=blockDim.y * blockDim.z*gridDim.x){
      ...
      if (threadIdx.x == 0){
        FeatureType J = float(intersection_size)/float(degu+(xadj[v+1]-xadj[v])-intersection_size);
